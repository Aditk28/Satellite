...
| T40 | **A DMM cannot see a rail that is failing a fast detector** | The Pi reported live under-voltage (`throttled=0x50005`, clamped to 600 MHz) while a multimeter at the Pi's own input read a steady 5.15 V under load. Not a contradiction: a DMM averages over ~100–300 ms and the Pi's monitor responds to dips orders of magnitude shorter. **Same blind spot as T6** — "a DC voltmeter verifies duty cycle, not pulse width" — and it cost most of a day here. Corollary that cost as much: **`throttled` bit meanings are easy to read backwards.** `0x50005` sets bits 0 and 2 (under-voltage NOW, throttled NOW) plus 16 and 18 (both have occurred since boot); `0x50000` is only the sticky history bits and means it happened *earlier*, which is success after a fix. And **600 MHz at idle is the normal `ondemand` floor, not evidence of throttling** — reading it as throttling produced a 2.33× extrapolation that was wrong by a factor of two (T33). **Poll the clock DURING load, and read bits 0 and 2, not the whole word.** |

| T41 | **Two subsystems, two positive-rotation conventions, and an identity that silently requires them to agree** | `psi = theta + psi_offset` was written as though the rotation controller's `theta` and vision's heading turned the same way. They do not: **`theta` is CW-positive, the dock frame is CCW-positive.** Measured twice by hand-rotating 30° CCW — `theta` −30.0/−30.1, `relyaw` +29.1/+26.9. **Why Phase 5 passed anyway is the whole lesson: every 5.1 test was STATIC.** With `theta` constant the offset absorbs any sign error and `psi` converges to the vision heading regardless, so `psi` tracked `relyaw` beautifully in every check that was run. The error only exists while TURNING — the gyro then contributes heading in the wrong direction, the offset has to chase 2× the rotation through a 0.02-per-frame filter, and `psi` is wrong for several seconds. Observed directly: after a `Z`, a 30° rotation left `psi` reading 17.0° where the identity predicts −22.3°, the offset having run 7.8° → 47.1° to compensate. That defeats the entire point of the T39 split *and* rotates the translation thrust vector. Fixed with a named `EST_THETA_SIGN` at the one place the conventions meet — **not** by touching `GYRO_SIGN`, which would invert a tuned, working axis. **A static test cannot validate a dynamic identity.** |
| T42 | **A good relative measurement plus a shaky absolute anchor is a shaky answer — and it fails looking exactly like a correct one** | The fan angle table was corrected by +180° on the strength of four single-fan `I50` accelerometer runs (which gave the fans' directions relative to each other correctly) anchored by two hand-shove `I0` captures (which appeared to put body-forward at +76° in the accelerometer's axes). The anchor was an artifact of picking a burst out of a sloppy record; the accelerometer frame is the body frame to within a few degrees. The "fix" swapped a 166° error for a 75° one and looked plausible the whole way. **The closed loop is the only measurement that samples the entire chain — estimator, transform, allocation, actuators — at once**, and it settles the anchor in one run: commanded body direction vs achieved `dv/dt`, with the magnitude confirming the actuators were doing their job. Right magnitude + wrong direction = a rotated table, and the rotation reads off directly. Anchor on the closed loop; keep the divergence guard armed while you do. |
| T43 | **A verbatim port that carries a sign without its reason** | Translation's Coulomb feedforward MOVING branch was ported from rotation as `-A_c*v_hat`. **The rotation minus comes entirely from the `-a` in `theta_ddot = -a*alpha`** — the wheel pushes the platform the other way. A fan pushes the platform the way it points, so translation needs `+A_c*v_hat`. Instead of cancelling drag it added it: the platform fought `1.9*A_c = 0.49 m/s^2` while moving, commanding −0.306 m/s² while needing +x and moving +x. The file's own PD comment said "no sign inversion here, unlike the wheel axis" — and the feedforward inverted it anyway, six lines later. **A second, cheaper check exists and should be applied to any future change here:** with the correct sign the STUCK and MOVING branches AGREE whenever the platform moves toward its target, so crossing `V_MOVING` is smooth; with the wrong sign they point 180° apart and every crossing reverses a 0.23 m/s² term. **A branch switch that is discontinuous is telling you one of the branches is wrong.** The verbatim rule protects a *reference*; it does not survive a change of plant. |
| T44 | **A safety guard whose threshold is derived from success** | The divergence guard tripped when `err > bestErr*1.5 + 0.03`. `bestErr` keeps falling, so the better the controller does the tighter the guard gets: the first genuinely working run converged to **1.6 mm**, which set the abort threshold to **32 mm**, and ordinary station-keeping drift then killed it — reported as `diverged`, i.e. the guard blamed the controller for its own success. The 15 s no-convergence timeout had the same defect from the other side; it would have killed that run 14 s *after* it converged. Fix: latch an `arrived` flag on first entry into the deadzone, then switch to a **fixed** runaway distance and stop applying the timeout. **A guard that fires because things went well is worse than no guard — it trains you to switch it off.** Related and separate: **a trip is not a stop.** The guard removes thrust; it cannot remove momentum, and at 0.30 m/s against 0.26 m/s² of friction the platform coasted ~17 cm after the fans went dead, into the wall. Choose start positions accordingly. |
...
| **B21** | **The staleness ladder is 3-tier and split by axis; tier 3 is deliberately NOT wired to `stopMotor()` in Phase 3** | 2026-08-19. **Why a ladder and not one timeout:** the two axes dead-reckon completely differently. Heading coasts on the gyro at ~0.8 °/min — 0.04° after 3 s, effectively free. Position coasts on double-integrated accelerometer, error ~½·a·t², so it is spent within about a second. Translation authority is therefore withdrawn a full 2 s before attitude control is even questioned: a platform holding heading with fans off is stable and recoverable, one that has also dropped attitude control just drifts. Thresholds **250 ms** (≈2–3 missed detections at 10 Hz — tolerate dropout without twitching) / **1 s** / **3 s**. **Why tier 3 only calls a hook:** nothing consumes pose until Phase 6, so a lost link endangers nothing today — while dumping a spinning flywheel *does* kick the platform at ~42 rad/s² against a 4.24 breakaway, and `safety.cpp`'s own comments warn against nuisance trips. A safe-stop that is not needed is not free on this machine. Tiers 1 and 2 fire for real; tier 3 announces itself so all three timers are exercised, and **Phase 6 repoints it at `stopMotor()`** once there is something to protect. **Why the ladder arms on the first valid frame:** if pose has never been valid its absence is not a loss — without that, every rotation test would trip a timeout at boot merely because no Pi is attached. **REVISED 2026-08-20 — see B21b, the original single-clock design was wrong.** |
| **B21b** | **POSE freshness and LINK health are tracked separately; only link silence is a fault** | 2026-08-20, forced by a hardware test: covering the tags reported `DEAD` while the link was perfectly healthy and frames were arriving with `crc=0`. B21 keyed **every** tier off "time since last VALID pose", which cannot distinguish *"I cannot see the dock"* from *"the Pi is gone"*. **Why that is severe rather than cosmetic: during SEARCH the tag is not visible by definition**, and B2 says it is routinely lost at close range too — so the terminal fault would have fired on every search sweep and every close approach, and would have surfaced in Phase 8 looking like an intermittent link problem three phases away from its cause. **The split:** *pose freshness* (last frame with `PI_FLAG_VALID`) drives `FRESH → STALE → LOST`, the estimator, and translation authority — **going stale is a normal operating condition**; *link health* (last frame of **any** kind, ≥ `PI_LINK_DEAD_MS` = 500 ms of silence ≈ 14 missed frames at 28 Hz) drives `DEAD` and the terminal hook — **silence is a fault**. Link death is checked first because it subsumes pose staleness. The mechanism to tell them apart already existed and was being thrown away: **the Pi deliberately sends frames with `PI_FLAG_VALID` clear when it cannot see the dock**, precisely so this is observable from the STM32. `G` now reports `poseage` and `linkage` separately. ⚠️ **Still wrong for Phase 6:** the `PI_LOST` action calls `fans_stopAll()`, the hard kill, which **latches until `R`** (B10) and only zeroes once — a running controller would simply re-command the fans next cycle. Phase 6 needs an **inhibit flag the allocator checks** plus a soft zero that leaves the ESC armed, so authority returns the instant a pose does. Safe today only because nothing commands the fans yet. |
