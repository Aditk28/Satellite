...
**Phases 0-5 COMPLETE. Phase 6 is WRITTEN BUT HAS NEVER BEEN RUN — that is the
next thing to do, and it is the largest remaining unknown in the project.**
...
| 6 translation control (LQR, x/y) | 🔶 **WRITTEN, NEVER RUN** | `src/translation.*` complete and building: PD in acceleration units (no mass term — the guide's `K_p = ω_n²·m` is wrong for this build, B14), Coulomb feedforward ported from rotation, dock→body rotation by `psi`, allocation onto two opposing fan pairs with 12% idle bias, square-law `throttle = √(A/2.1e-4)`. Commands `TX`/`TY`/`TT`/`TS`. **Divergence guard** kills the fans if error grows under thrust (catches a wrong fan angle, backwards prop, reversed ESC, or a frame-sign slip without caring which) plus a 15 s no-convergence timeout. **NOTHING HAS BEEN TESTED ON HARDWARE.** |
